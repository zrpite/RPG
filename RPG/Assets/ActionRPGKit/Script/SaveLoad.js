	...
	if(menu){
		...
		if(GUI.Button (Rect (Screen.width / 2 - 45,385,90,40), "Quit Game")){
			if(GlobalCondition.freezePlayer){
				player.transform.root.SendMessage("Deactivate" , SendMessageOptions.DontRequireReceiver);
			}
			var cam : GameObject = GameObject.FindWithTag ("MainCamera");
			Destroy(cam);
			if(player.GetComponent(UIiMaster)){
				player.GetComponent(UIiMaster).DestroyAllUi();
			}
			Destroy(player);
			Time.timeScale = 1.0;
...
function SaveData(){
		...
		//	PlayerPrefs.SetInt("PlayerMP", player.GetComponent(Status).mana);
			...
			PlayerPrefs.SetInt("PlayerSKP" +saveSlot.ToString(), player.GetComponent(Status).skillPoint);
			...
			PlayerPrefs.SetInt("SubEquip" +saveSlot.ToString(), player.GetComponent(Inventory).subWeaponEquip);
			PlayerPrefs.SetInt("ArmoEquip" +saveSlot.ToString(), player.GetComponent(Inventory).armorEquip);
			PlayerPrefs.SetInt("AccEquip" +saveSlot.ToString(), player.GetComponent(Inventory).accessoryEquip);
			//Save Quest
			...
			//Global Event
			a = 0;
			while(a < GlobalCondition.eventVar.Length){
				PlayerPrefs.SetInt("GlobalVar" + a.ToString() +saveSlot.ToString(), GlobalCondition.eventVar[a]);
				a++;
			}
...
function LoadData(){
		...
		respawn.GetComponent(Status).skillPoint = PlayerPrefs.GetInt("PlayerSKP" +saveSlot.ToString());
		...
		var itemSize : int = player.GetComponent(Inventory).itemSlot.length;
			...
			respawn.GetComponent(Inventory).subWeaponEquip = PlayerPrefs.GetInt("SubEquip" +saveSlot.ToString());
			respawn.GetComponent(Inventory).armorEquip = PlayerPrefs.GetInt("ArmoEquip" +saveSlot.ToString());
			respawn.GetComponent(Inventory).accessoryEquip = PlayerPrefs.GetInt("AccEquip" +saveSlot.ToString());
		...
		var questSlotSize : int = respawn.GetComponent(QuestStat).questSlot.length;
			...
			a = 0;
			while(a < GlobalCondition.eventVar.Length){
				GlobalCondition.eventVar[a] = PlayerPrefs.GetInt("GlobalVar" + a.ToString() +saveSlot.ToString());
				a++;
			}
		...
		lastPosition.y = PlayerPrefs.GetFloat("PlayerY" +saveSlot.ToString()) + 0.5;
		lastPosition.z = PlayerPrefs.GetFloat("PlayerZ" +saveSlot.ToString());
		respawn.transform.position = lastPosition;
		var sceneLoad : int = PlayerPrefs.GetInt("Scene" +saveSlot.ToString());
...
function LoadGame(){
		...
		player.GetComponent(Status).skillPoint = PlayerPrefs.GetInt("PlayerSKP" +saveSlot.ToString());
		
		player.GetComponent(Status).CalculateStatus();
		player.GetComponent(Status).health = player.GetComponent(Status).totalMaxHealth;
		player.GetComponent(Status).mana = player.GetComponent(Status).totalMaxMana;
		//mainCam = GameObject.FindWithTag ("MainCamera").transform;
		//mainCam.GetComponent(ARPGcamera).target = respawn.transform;
		//-------------------------------
		...
		var itemSize : int = player.GetComponent(Inventory).itemSlot.length;
			...
			player.GetComponent(Inventory).subWeaponEquip = PlayerPrefs.GetInt("SubEquip" +saveSlot.ToString());
			player.GetComponent(Inventory).armorEquip = PlayerPrefs.GetInt("ArmoEquip" +saveSlot.ToString());
			player.GetComponent(Inventory).accessoryEquip = PlayerPrefs.GetInt("AccEquip" +saveSlot.ToString());
		if(PlayerPrefs.GetInt("WeaEquip" +saveSlot.ToString()) == 0){
