...
function OnTriggerEnter (other : Collider) {  	
    ...
    if(shooterTag == "Player" && other.tag == "Enemy"){
		...
		if(drainHp > 0 && shooter && popDamage != "Miss" && popDamage != "Evaded" && popDamage != "Guard" && popDamage != "Invulnerable"){
			var lf : int = parseInt(popDamage) * drainHp;
			lf /= 100;
			if(lf < 1){
				lf = 1;
			}
			var hpos : Vector3 = shooter.transform.position;
			hpos.y += 0.75;
			var hpPop : Transform = Instantiate(Popup, hpos , transform.rotation);
			hpPop.GetComponent.<DamagePopup>().damage = lf.ToString();
			hpPop.GetComponent.<DamagePopup>().fontStyle.normal.textColor = Color.green;
		...
		dmgPop.GetComponent(DamagePopup).damage = popDamage;
 		 ...
 		 //Drain HP
		if(drainHp > 0 && shooter && popDamage != "Miss" && popDamage != "Evaded" && popDamage != "Guard" && popDamage != "Invulnerable"){
			lf = parseInt(popDamage) * drainHp;
			lf /= 100;
			if(lf < 1){
				lf = 1;
			}
			hpos = shooter.transform.position;
			hpos.y += 0.75;
			hpPop = Instantiate(Popup, hpos , transform.rotation);
			hpPop.GetComponent.<DamagePopup>().damage = lf.ToString();
			hpPop.GetComponent.<DamagePopup>().fontStyle.normal.textColor = Color.green;
			shooter.GetComponent.<Status>().Heal(lf , 0);
		}
