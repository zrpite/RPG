...
var keepBodyOnMap : boolean = false;
...
function LoadData(){
		...
		respawn.GetComponent(Status).skillPoint = PlayerPrefs.GetInt("TempPlayerSKP");
		mainCam = GameObject.FindWithTag ("MainCamera").transform;
		...
		var itemSize : int = player.GetComponent(Inventory).itemSlot.length;
			...
			respawn.GetComponent(Inventory).subWeaponEquip = PlayerPrefs.GetInt("TempSubEquip");
			respawn.GetComponent(Inventory).armorEquip = PlayerPrefs.GetInt("TempArmoEquip");
			respawn.GetComponent(Inventory).accessoryEquip = PlayerPrefs.GetInt("TempAccEquip");
