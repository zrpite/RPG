...
function Activate () {
	...
	if(open == GuiType.Inventory){
		if(!player.GetComponent(Inventory).useLegacyUi && player.GetComponent(UIiMaster)){
			player.GetComponent(UIiMaster).OnOffInventoryMenu();
		}else{
			player.GetComponent(Inventory).OnOffMenu();
		}
	}else if(open == GuiType.Skill){
		if(!player.GetComponent(SkillWindow).useLegacyUi && player.GetComponent(UIiMaster)){
			player.GetComponent(UIiMaster).OnOffSkillMenu();
		}else{
			player.GetComponent(SkillWindow).OnOffMenu();
		}
	}else if(open == GuiType.Status){
		if(!player.GetComponent(StatusWindow) && player.GetComponent(UIiMaster) || !player.GetComponent(StatusWindow).enabled && player.GetComponent(UIiMaster)){
			player.GetComponent(UIiMaster).OnOffStatusMenu();
		}else{
