...
function UseItem(slot : int){
	...
	GetComponent(Status).atk += dataItem.usableItem[id].atkPlus;
	GetComponent(Status).def += dataItem.usableItem[id].defPlus;
	GetComponent(Status).matk += dataItem.usableItem[id].matkPlus;
	GetComponent(Status).mdef += dataItem.usableItem[id].mdefPlus;
...
function EquipItem(id : int , slot : int){
	...
	if(dataItem.equipment[id].EquipmentType == dataItem.equipment[id].EqType.Weapon){
		...
		if(dataItem.equipment[id].attackPrefab){
			GetComponent(AttackTrigger).attackPrefab = dataItem.equipment[id].attackPrefab.transform;
		}
		//Change Weapon Mesh
	...
	}else if(dataItem.equipment[id].EquipmentType == dataItem.equipment[id].EqType.Armor){
		//Armor Type
...
function SwapWeapon(){
	var tempEq : int = weaponEquip; //Store Main Weapon Data
...
function SettingEquipmentStatus(){
	var dataItem : ItemData = database.GetComponent(ItemData);
...
function UnEquip(id : int){
	...
	if(!full){
		if(dataItem.equipment[id].EquipmentType == dataItem.equipment[id].EqType.Weapon){
			weaponEquip = 0;
			GetComponent(AttackTrigger).attackPrefab = fistPrefab.transform;
			if(weapon){
				var allWeapon : int = weapon.length;
				var a : int = 0;
				if(allWeapon > 0){
...
function AssignWeaponAnimation(id : int){
	var dataItem : ItemData = database.GetComponent(ItemData);
	var playerAnim : PlayerAnimation = GetComponent(PlayerAnimation);
	if(!playerAnim){
		//If use Mecanim
	...
	if(dataItem.equipment[id].attackCombo && dataItem.equipment[id].EquipmentType == dataItem.equipment[id].EqType.Weapon){
  			...
  			GetComponent(AttackTrigger).attackCombo = new AnimationClip[allPrefab];
  
  			var a : int = 0;
  			if(allPrefab > 0){
   				while(a < allPrefab){
    				GetComponent(AttackTrigger).attackCombo[a] = dataItem.equipment[id].attackCombo[a];
    				GetComponent(AttackTrigger).mainModel.GetComponent.<Animation>()[dataItem.equipment[id].attackCombo[a].name].layer = 15;
    				a++;
   				}
  			}
  			GetComponent(AttackTrigger).whileAttack = parseInt(dataItem.equipment[id].whileAttack);
  			//Assign Attack Speed
  			GetComponent(AttackTrigger).attackSpeed = dataItem.equipment[id].attackSpeed;
  			GetComponent(AttackTrigger).atkDelay1 = dataItem.equipment[id].attackDelay;
 		}
...
function AssignMecanimAnimation(id : int){
	...
	if(dataItem.equipment[id].EquipmentType == dataItem.equipment[id].EqType.Weapon){
			GetComponent(AttackTrigger).whileAttack = parseInt(dataItem.equipment[id].whileAttack);
  			...
  			GetComponent(PlayerMecanimAnimation).SetWeaponType(dataItem.equipment[id].weaponType , dataItem.equipment[id].idleAnimation.name);
