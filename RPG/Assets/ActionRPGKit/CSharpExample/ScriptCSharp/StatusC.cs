...
public class StatusC : MonoBehaviour {
	...
	[System.Serializable]
	public class LaunchedAnimationSet{
		public AnimationClip launchedAnimation;
		public AnimationClip knockdownAnimation;
		public AnimationClip getUpAnimation;
	}
	public LaunchedAnimationSet launchAnimationSet;

	void OnControllerColliderHit(ControllerColliderHit hit){
		if(launched && hit.gameObject.tag != "Enemy" && hit.gameObject.tag != "Player"){
			if(launchAnimationSet.launchedAnimation){
				mainModel.GetComponent<Animation>().Stop(launchAnimationSet.launchedAnimation.name);
			}
			...
			if(floorHitEffect){
				Instantiate(floorHitEffect , transform.position , transform.rotation);
			}
			//StartCoroutine("KnockDown" , 2.0f);
			//KnockDown(2.0f);
			KnockDown(downDuration);
		}
		//--------------------------------------------------
	}
	
	public void BounceUp(float ktime , float dtime){
		if(launched || !launchAnimationSet.launchedAnimation || freeze || dead){
			return;
		}
		launched = true; // Gagaga
		downDuration = dtime;
		down = false;
		freeze = true;
		if(launchAnimationSet.knockdownAnimation){
			mainModel.GetComponent<Animation>().Stop(launchAnimationSet.knockdownAnimation.name);
		}
		mainModel.GetComponent<Animation>()[launchAnimationSet.launchedAnimation.name].layer = 35;
		mainModel.GetComponent<Animation>().PlayQueued(launchAnimationSet.launchedAnimation.name , QueueMode.PlayNow);
		StartCoroutine(BounceDown(ktime));
	}
	
	public IEnumerator BounceDown(float ktime){
		yield return new WaitForSeconds(ktime);
		launched = true;
	}

	public void KnockDown(float ktime){
		if(launched && down){
			//----------------------
			down = false;
		}
		//---------------------
		if(down || !launchAnimationSet.knockdownAnimation || !launchAnimationSet.getUpAnimation || dead){
			return;
		}
		StartCoroutine("GetUp" , ktime);
	}
	
	public IEnumerator GetUp(float ktime){
		launched = false;
		down = true;
		//freeze = true;
		int clayer = gameObject.layer;
		gameObject.layer = 2;
		if(launchAnimationSet.knockdownAnimation){
			mainModel.GetComponent<Animation>()[launchAnimationSet.knockdownAnimation.name].layer = 17;
			mainModel.GetComponent<Animation>().PlayQueued(launchAnimationSet.knockdownAnimation.name, QueueMode.PlayNow);
		}
		down = true;
		yield return new WaitForSeconds(ktime);
		if(launchAnimationSet.knockdownAnimation){
		...
		if(launchAnimationSet.getUpAnimation && down){
			if(launchAnimationSet.launchedAnimation){
				mainModel.GetComponent<Animation>().Stop(launchAnimationSet.launchedAnimation.name);
			}
			//mainModel.animation.Stop(knockdownAnimation);
			mainModel.GetComponent<Animation>()[launchAnimationSet.getUpAnimation.name].layer = 10;
			mainModel.GetComponent<Animation>().PlayQueued(launchAnimationSet.getUpAnimation.name , QueueMode.PlayNow);
			float wait = mainModel.GetComponent<Animation>()[launchAnimationSet.getUpAnimation.name].length;
			yield return new WaitForSeconds(wait);
		}
		gameObject.layer = clayer;
		down = false;
		freeze = false;
		print("asdas");
	}
